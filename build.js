  ...
  highlight: function (str, lang) {
    ...
    return (
      ...
      code
        ...
        .split('\n')
        .map(
          x => `<span class='code'><span class='codeline'>${x}</span></span>\n`
        )
        .join('') +
      '</code></pre>'
    )
...
fs.writeFile(
  'pages/data/category.json',
  JSON.stringify(categoryLinks),
  () => {}
)
...
for (var i = 0; i < posts.length; i++) {
  ...
  const n =
    j === sortedPosts[category].length - 1 ? ' ' : sortedPosts[category][j + 1]
  ...
  fs.writeFile(
    ...
    pug.renderFile('pugs/post.pug', {
      ...
      date: `${post.slice(0, 2)}/${post.slice(2, 4)}/${post.slice(
        4,
        6
      )} ${post.slice(6, 8)}:${post.slice(8, 10)}`
    }),
    () => {}
  )
}
...
var preview, date
const length = 300
for (const post of posts.length < 3 ? posts : posts.slice(0, 3)) {
...
fs.writeFileSync(
  'pages/index.html',
  pug.renderFile('./pugs/index.pug', { posts: recentPosts })
)
...
function categoryPosts (categoryCode, posts) {
  ...
  for (var i = 0; i < pagelength; i++) {
    ...
    fs.writeFileSync(
      `pages/posts/${categoryCode}/${i}.html`,
      pug.renderFile('./pugs/postlist.pug', {
  ...
  if (pagelength == 0) {
    fs.writeFileSync(
      `pages/posts/${categoryCode}/0.html`,
      pug.renderFile('./pugs/postlist.pug', {
        selected: categoryCode,
        allposts: allposts,
        categorylist: categorylist,
        posts: [],
        now: { number: 0, url: `/posts/0/0.html` },
        after: [],
        before: [],
        categories: categoryLinks
      })
...
fs.writeFileSync(
  'pages/search.html',
  pug.renderFile('./pugs/search.pug', { categorylist: _categories })
)
...
for (const category of Object.keys(sortedPosts)) {
  ...
  for (const divided of divide(sortedPosts[category], 30)) {
    fs.writeFile(
      'pages/data/' + category + '/' + (n++).toString() + '.json',
